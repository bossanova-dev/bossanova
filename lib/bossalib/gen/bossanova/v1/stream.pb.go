...
func (x *OrchestratorCommand) GetMoveSession() *MoveSessionCommand {
	if x != nil {
		if x, ok := x.Cmd.(*OrchestratorCommand_MoveSession); ok {
			return x.MoveSession
		}
	}
	return nil
}
...
type OrchestratorCommand_MoveSession struct {
	// bosso → daemon: move a session up or down the rendered list. The rank
	// arithmetic lives entirely in the daemon, so bosso forwards the direction
	// and never computes a position. Replies CommandResult{move_session}.
...
func (x *CommandResult) GetMoveSession() *MoveSessionResponse {
	if x != nil {
		if x, ok := x.Payload.(*CommandResult_MoveSession); ok {
			return x.MoveSession
		}
	}
	return nil
...
type CommandResult_MoveSession struct {
	// MoveSession returns the moved session plus is_moved (reuses daemon.proto
	// type). It does NOT reuse session = 4: a boundary move is a successful
	// no-op that must stay distinguishable from a move that changed the order,
	// and the bare Session payload has nowhere to say so.
...
// MoveSessionCommand asks the daemon to move a session one position up or down
// the rendered list. Replies with CommandResult carrying a MoveSessionResponse
// (move_session = 41).
//
// It mirrors DaemonService.MoveSession field for field rather than reusing
// MoveSessionRequest, because every command in this oneof names its subject
// session_id and a lone `id` here would read as the command's own identity.
// The daemon owns the rank arithmetic: this command carries a DIRECTION, never
// a computed position, so two clients can never derive different ranks.
type MoveSessionCommand struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The session to move.
	SessionId string `protobuf:"bytes,1,opt,name=session_id,json=sessionId,proto3" json:"session_id,omitempty"`
	...
	// so the neighbours the daemon reasons about are the ones the caller is
	// actually looking at.
...
func (x *MoveSessionCommand) Reset() {
	*x = MoveSessionCommand{}
	mi := &file_bossanova_v1_stream_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MoveSessionCommand) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MoveSessionCommand) ProtoMessage() {}

func (x *MoveSessionCommand) ProtoReflect() protoreflect.Message {
	mi := &file_bossanova_v1_stream_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use MoveSessionCommand.ProtoReflect.Descriptor instead.
func (*MoveSessionCommand) Descriptor() ([]byte, []int) {
	return file_bossanova_v1_stream_proto_rawDescGZIP(), []int{30}
}

func (x *MoveSessionCommand) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
...
func (x *MoveSessionCommand) GetDirection() MoveDirection {
	if x != nil {
		return x.Direction
	}
...
func (x *MoveSessionCommand) GetRepoId() string {
