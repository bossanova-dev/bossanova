...
# BOS-373: `test` is the fast default (SHORT=1 COVERAGE=0) — the per-service inner
# loop. `test-all` is the exhaustive coverage run (COVERAGE=1). `test-fast` is a
# fast alias kept for back-compat (root `test-smoke` fallback + docs).
...
.PHONY: test test-all test-fast go-test
