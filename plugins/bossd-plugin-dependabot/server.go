...
func (s *server) PollTasks(ctx context.Context, req *bossanovav1.PollTasksRequest) (*bossanovav1.PollTasksResponse, error) {
	...
	// Prefer the owner/repo slug for log context; fall back to the raw URL
	// when the URL doesn't parse so logs never go silent.
	repo := vcs.RepoSlug(repoURL)
	if repo == "" {
		repo = repoURL
	}
	...
	if err != nil {
		s.logger.Warn().Err(err).Str("repo", repo).Msg("failed to list closed PRs, skipping rejection check")
		closedPRs = nil
	}
	...
	for _, pr := range prs {
		...
		if isPreviouslyRejected(pr, closedPRs) {
			...
			tasks = append(tasks, &bossanovav1.TaskItem{
				...
				// BaseBranch is intentionally left empty: the daemon knows the
				// repo's configured default branch and fills it in. Plugins
				// that hardcode "main" misdirect repos using master/develop/trunk.
				BaseBranch:     "",
		...
		task, err := s.classifyPR(ctx, repoURL, repo, pr)
		if err != nil {
...
// repo is the "owner/name" slug for log context; repoURL is the canonical URL used for host calls.
func (s *server) classifyPR(ctx context.Context, repoURL, repo string, pr *bossanovav1.PRSummary) (*bossanovav1.TaskItem, error) {
	...
	if len(checks) == 0 {
		s.logger.Debug().Str("repo", repo).Int32("pr", prNumber).Msg("no checks yet, skipping")
		return nil, nil
	}
	...
	base := &bossanovav1.TaskItem{
		...
		Title:         pr.GetTitle(),
		RepoOriginUrl: repoURL,
		// BaseBranch intentionally empty — the daemon fills it in from the
		// repo's configured default so master/develop/trunk repos don't get
		// misdirected. See handleCreateSession in orchestrator.go.
		BaseBranch:     "",
		ExistingBranch: pr.GetHeadBranch(),
		Labels:         []string{"dependabot", parseDependabotLibrary(pr)},
	}
	...
	if prStatus.Mergeable != nil && !*prStatus.Mergeable {
		s.logger.Debug().Str("repo", repo).Int32("pr", prNumber).Msg("not mergeable, skipping")
		return nil, nil
	}

	// Checks passed + mergeable → AUTO_MERGE.
	s.logger.Info().Str("repo", repo).Int32("pr", prNumber).Str("title", pr.GetTitle()).Msg("checks passed, auto-merge")
