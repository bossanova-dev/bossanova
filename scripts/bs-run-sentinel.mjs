...
// of scripts/bs-review-caps.mjs (module + thin CLI + a byte-stable token set
// pinned by scripts/bs-run-sentinel.test.mjs).
...
//   node scripts/bs-run-sentinel.mjs make-ctx <skill> [tmpdir]  → prints `runId\tdir`
//   node scripts/bs-run-sentinel.mjs write <dir> <runId> <name> <kind> [payloadJson]
//   node scripts/bs-run-sentinel.mjs read  <dir> <runId> <name>  → prints the JSON result
//   node scripts/bs-run-sentinel.mjs match <token>              → JSON ({result}|null)
//   node scripts/bs-run-sentinel.mjs cleanup <dir>
//
