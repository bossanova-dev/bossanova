...
// Pure, tracker-agnostic epic-decomposition primitive for boss-plan (BOS-442).
//
...
// Modelled on scripts/linear-deps-lib.mjs and skills-toolbox/dag-scheduler.mjs.
//
...
// `priority` a real Todo priority (1=Urgent … 4=Low; 0=None is not a planned
...
export function validateDecomposition(spec) {
  ...
  children.forEach((child, i) => {
    ...
    // Planning metadata the epic-child spec requires (Phase 2.5 creates Todo
    // children from it): a non-empty keyChanges, a Fibonacci estimate, a real
    // Todo priority. Missing/out-of-range values are rejected BEFORE any write
...
// move the parent out of Unplanned, so the parent-repurpose-LAST atomicity guard
