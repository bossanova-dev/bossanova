...
function defaultRenderStill({ input, output, title }) {
  const [command, args] = terminalRenderCommand({
  ...
  if (result.status !== 0) {
    throw new Error(`terminal render failed (${command} exited ${result.status})`);
  }
}
 ...
 * with an install hint, mirroring proof.mjs's vhs-missing skip. The shared
...
export async function runTuiAgentProof({
  ...
  // captured and the agent passed + evidence gate passed (mirrors proof.mjs's
  // vhs-skip semantics: a stills-only proof is still a pass). hasFailure trips
  ...
  return finalizeAgentProof({
    ...
    shouldUpload,
    bucket,
    // Decision CM2b: scan the TUI surface texts (final settled screen + brief
    // title/description + agent summary) — NOT the .cast (fixture-only TUI). The
    // manifest is always scanned inside finalize.
 ...
 */
async function runAgentLoop({ brief, model, modelDep, bridge, rawDir }) {
  const goal = [
    ...
    for (const block of resp.content ?? []) {
      ...
      if (block.name === 'done') {
        ...
        try {
          ...
          screenN += 1;
          fs.writeFileSync(
            path.join(rawDir, `screen-${String(screenN).padStart(2, '0')}.txt`),
            screen,
          );
          toolResult = { screen };
        } catch (err) {
...
async function renderFrames({ rawDir, captureDir, title, renderStill }) {
  ...
  for (const sf of screenFiles) {
    ...
    try {
      ...
      await renderStill({ input: path.join(rawDir, sf), output, title });
    } catch (err) {
