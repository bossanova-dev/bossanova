...
test('dedupeAgainstPRs drops alerts already covered by an open dependabot PR or sweep sentinel', () => {
  const alerts = [mkAlert({ number: 1, pkg: 'lodash' }), mkAlert({ number: 2, pkg: 'axios', ghsa: 'GHSA-y' })];
  const prs = [
  ...
  assert.deepEqual(dedupeAgainstPRs(alerts, prs).map((a) => a.number), []);
});
...
test('renderState/parseState round-trips and embeds ghsa dedupe sentinels', () => {
  const body = renderState({ attempts: 1, lastSha: 'abc1234', lastOutcome: 'retry', batchGhsas: ['GHSA-a', 'GHSA-b'], updatedAt: '2026-06-25T00:00:00Z' });
...
test('runCli select-batch reads files via injected reader and prints JSON', () => {
  const files = {
    'a.json': JSON.stringify([
      { number: 1, security_advisory: { severity: 'high', ghsa_id: 'GHSA-x' },
        dependency: { scope: 'runtime', package: { ecosystem: 'npm', name: 'lodash' }, manifest_path: 'pnpm-lock.yaml' },
  ...
  assert.deepEqual(parsed.batch.map((a) => a.number), [1]);
});
