...
// `make vendor-toolbox-check` pins the five vendored copies byte-identical to this file, and
// `copy-skills` — a prerequisite of the test targets — regenerates the plugin mirror from those
// copies, so reading the canonical one covers them all. (The check itself walks only the two
// authoring roots; it never inspects the plugin mirror.)
...
func TestExtensionContractDocumentsPathLoading(t *testing.T) {
	...
	for _, want := range []struct{ phrase, why string }{
		...
		// BOS-798: the contract used to call the marker check "the single place to relax", which is
		// true of the AUTHORING site and false of the edit: `make vendor-toolbox` re-copies
		// skill-extensions.mjs into the five committed vendored toolboxes and, via its chained
		// `copy-skills` leg, the plugin mirror. A reader who took the old sentence at its word
		// edited one file and shipped a stale tree. The corrected sentence must not over-claim the
		// remedy either: `vendor-toolbox-check` walks only the two authoring roots, so naming it as
		// the gate that "reds until" the mirror is re-copied hands a follower a false green on the
		// exact site the passage warns about. Both halves are pinned — the corrected claim about
		// this remedy, and the general rule it generalises to, because a remedy stated here is
		// copied into every skill derived from it.
		{
			"but it is not a single-site edit",
			"must not leave the marker-check remedy reading as a one-file edit — the canonical copy is re-vendored into every committed toolbox and the plugin mirror",
		},
		{
			"Enumerate the readers before you edit, and re-vendor in the same commit",
			"must state the remedy for the multi-site edit, not merely warn that it is one",
		},
		{
			"a stale plugin mirror passes it green",
			"must scope the vendor gate to what it actually walks — `vendor-toolbox-check` never inspects plugins/bossd-plugin-claude/skilldata, so naming it as the gate that reds until the mirror is re-copied gives a follower a false green on the very site this passage warns about",
		},
		{
			"regenerated by `copy-skills`, a prerequisite of the test targets",
			"must name what actually refreshes the plugin mirror, so a reader who is told the check does not cover it is not left without a remedy",
		},
		{
			"This is the general rule for every remedy sentence in this document",
			"must generalise the upstream-grep rule to the whole document — scoped to one paragraph it re-seeds the identical defect through the next remedy a reviewer quotes",
		},
		{
			"grep the quoted remedy across the whole skills tree",
			"must prescribe the grep that finds the other sites, not merely assert that they exist",
		},
		{
			"cite the old wording as their **reason** rather than restating it",
			"must require the rationale sweep as well as the restatement sweep — restatements are greppable and rationales are not, so the half that rots is the half no grep hands you",
		},
	} {
		if !strings.Contains(flatContract, want.phrase) {
	...
	for _, banned := range []struct{ phrase, why string }{
		...
		{
			"`make vendor-toolbox-check` reds until it has",
			"must not claim the vendor check covers the plugin mirror — it walks only the two authoring roots, so the claim is a false green on the site the sentence is warning about",
		},
		{
			"is the single place to relax if pure-name discovery",
			"must not restate the superseded single-site claim — a reader who greps for the marker-check remedy and finds it still standing has been handed back the exact wrong answer this fix removed",
		},
	} {
