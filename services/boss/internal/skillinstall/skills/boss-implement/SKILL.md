...
**only when their trigger fires** — the read-when-triggered pattern gstack/plan-eng-review use. The
...
| `references/subagent-driven-development.md` | Step 5, before Task 1 — implementer spine                     |
| `references/test-driven-development.md`     | Step 5 — implementer TDD discipline                           |
...
The first four are generated from the superpowers plugin by `make construct-skills` (do not edit them
directly — edit `SKILL.md.tmpl` + `construct.json`); the rest are hand-written.
...
```bash
LOCK="$(git rev-parse --show-toplevel)/.claude/skills/boss-implement/toolbox/worktree-lock.sh"
...
## Step 5: Implement — subagent-driven development (TOP-LEVEL SPINE)
...
**Every dispatched subagent — implementer, task reviewer, fix subagent — is awaited; never
`run_in_background`.** Subagent-driven development is sequential by design.
...
task's full transcript. The spine's file-handoff rules (task brief / report file / review package as
files) still apply.

**Read [`references/subagent-driven-development.md`](references/subagent-driven-development.md) now** —
before dispatching Task 1 (fresh implementer per task, per-task spec+quality review, model selection,
file handoffs, durable progress ledger). Each implementer follows TDD: write the failing test first,
watch it fail, then minimal code to pass — read
[`references/test-driven-development.md`](references/test-driven-development.md) when a task's shape is
non-obvious.
...
```bash
RUN_SENTINEL="$(git rev-parse --show-toplevel)/.claude/skills/boss-implement/toolbox/bs-run-sentinel.mjs"
...
export RUN_SENTINEL RUN_ID RUN_DIR DISPATCH_FAILURE
```
...
```bash
"$(git rev-parse --show-toplevel)/.claude/skills/boss-implement/toolbox/worktree-lock.sh" release "$BLI_RUNID"
```
...
`node .claude/skills/boss-implement/gate/gate.mjs` on the job (scheduler UI, `GateCommand`) so the run
