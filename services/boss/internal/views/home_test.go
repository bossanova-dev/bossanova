...
func TestMergingOverrideClearedWhenSessionGone(t *testing.T) {
	// The merging override is dropped when its own row leaves the session poll,
	// mirroring the archiving reconcile.
	h := NewHomeModel(nil, context.Background(), nil)
	h.markMerging("s1")
	h.markMerging("s2")
	...
	if got.isMerging("s1") || got.mergeInFlight("s1") {
		t.Fatal("expected s1 merge state cleared when s1 is absent")
	}
	if !got.isMerging("s2") || !got.mergeInFlight("s2") {
		t.Fatal("expected s2 merge state retained when s2 remains")
	}
}

func TestRenderSessionStatusMergingOverride(t *testing.T) {
	h := HomeModel{
		mergingOverrideIDs: map[string]struct{}{"s1": {}, "s2": {}},
		spinner:            newStatusSpinner(),
	}
	...
	for _, id := range []string{"s1", "s2"} {
		...
		if !strings.Contains(got, "merging") {
			t.Fatalf("renderSessionStatus for merging session %s = %q, want to contain %q", id, got, "merging")
		}
		if !strings.Contains(got, h.spinner.View()) {
			t.Fatalf("renderSessionStatus for merging session %s = %q, want spinner frame", id, got)
		}
	}
	...
	want := renderDisplayStatus(other, h.spinner)
	if got2 := h.renderSessionStatus(other); got2 != want {
		t.Fatalf("renderSessionStatus for non-merging session = %q, want passthrough %q", got2, want)
	}
}

// TestRenderSessionStatusMergingTakesPrecedenceOverArchiving guards the
// ordering in renderSessionStatus: a session flagged both merging and archiving
// (an unusual concurrent state) renders the merging label, since the merging
// check runs first.
func TestRenderSessionStatusMergingTakesPrecedenceOverArchiving(t *testing.T) {
	h := HomeModel{
		mergingOverrideIDs:   map[string]struct{}{"s1": {}},
		archivingOverrideIDs: map[string]struct{}{"s1": {}},
		spinner:              newStatusSpinner(),
	}
	got := h.renderSessionStatus(&pb.Session{Id: "s1", DisplayStatus: pb.DisplayStatus_DISPLAY_STATUS_PASSING})
	if !strings.Contains(got, "merging") {
		t.Fatalf("renderSessionStatus = %q, want merging to take precedence", got)
	}
	if strings.Contains(got, "archiving") {
		t.Fatalf("renderSessionStatus = %q, want merging not archiving", got)
	}
}

func TestMergeStateResolveClearsBothSets(t *testing.T) {
	// resolveMerge drops the id from BOTH the override and in-flight sets,
	// regardless of outcome (the caller hands off to mergedOptimisticID on
	// success and rebuilds rows on failure).
	h := NewHomeModel(nil, context.Background(), nil)
	h.markMerging("s1")
	h.markMerging("s2")
	if !h.isMerging("s1") || !h.mergeInFlight("s1") {
		t.Fatal("precondition: expected s1 marked merging + in flight")
	}

	h.resolveMerge("s1")

	if h.isMerging("s1") || h.mergeInFlight("s1") {
		t.Fatal("expected resolveMerge to clear both merging sets for s1")
	}
	if !h.isMerging("s2") || !h.mergeInFlight("s2") {
