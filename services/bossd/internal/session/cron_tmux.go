...
//     (plus --dangerously-skip-permissions when the claude plugin's
//     "dangerously_skip_permissions" config entry is "true") in the
//     worktree path.
//  4. Persist an agent_chats row titled `Run "<cron name>"`, then stamp the
...
func (l *Lifecycle) startCronTmuxChat(
	...
	if l.agentChats == nil {
		// A nil AgentChatStore would leave the tmux session orphaned with no
		...
		return "", fmt.Errorf("agentChats store unavailable: cannot host cron-spawned session %s", sessionID)
	}

	// Step 2: mint a fresh Claude session UUID and derive the tmux name.
	agentSessionID := uuid.NewString()
	tmuxName := tmux.ChatSessionName(session.RepoID, agentSessionID)

	// Step 3: build the claude command line and spawn the tmux session.
	args := []string{"claude", "--session-id", agentSessionID}
	...
	if cfgErr != nil {
		...
		// from cron runs even when the operator has enabled it via the
		// claude plugin's "dangerously_skip_permissions" config entry.
		l.logger.Warn().Err(cfgErr).
			Str("session", sessionID).
			Msg("cron: failed to load config; defaulting (claude plugin dangerously_skip_permissions unset)")
	}
	...
	// Step 4: persist the agent_chats row, then stamp the tmux name on it.
	...
	if _, err := l.agentChats.Create(ctx, db.CreateAgentChatParams{
		...
		l.killCronTmuxBestEffort(ctx, sessionID, agentSessionID, tmuxName)
		return "", fmt.Errorf("create agent_chats row for cron session %s: %w", sessionID, err)
	}
...
func (l *Lifecycle) killCronTmuxBestEffort(ctx context.Context, sessionID, agentSessionID, tmuxName string) {
	if l.tmux == nil {
		return
	}
