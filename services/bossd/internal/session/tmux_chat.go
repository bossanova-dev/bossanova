...
// agent-grammar half — the ModalDetector — is bound per call from the chat's own
// runner client, which is why this function takes one: the daemon holds a single
// long-lived tmux.Client shared by every chat, so the caller is the only layer
// that knows which agent owns this pane.
//
// The second half is here because row-anchoring alone does NOT refuse the
// incident BOS-600 was opened for, and BOS-894 settled that by capturing the
// pane instead of reasoning about it. codex's "Update available!" interstitial
// draws row 1 as "› 1. Update now" — its own composer glyph — so
// composerRowIndex resolves it as a live input row, delivery types its payload,
// and the Enter that follows selects "Update now": codex reinstalls itself over
// the running binary and the pane dies with the chat in it. The capture is
// committed at plugins/bossd-plugin-codex/testdata/panes/update_interstitial.txt
// and regenerated by capture-update-interstitial.sh beside it; the grammar that
// recognises it is hasCodexBootInterstitial in that plugin.
//
// The rule that produced this is worth keeping, because it is what an earlier
// version of this comment got right while getting the conclusion wrong: capture
// the rendering first, and where it has not been captured, disclose the gap
// rather than guess at what the pane looks like. The gap that comment disclosed
// was real; what closed it was going and capturing the screen.
//
// What this does NOT yet cover, stated here because the plumbing looks
// agent-agnostic and therefore looks finished: only codex has a boot-screen
// clause. statusdetect.HasModalPrompt, which the claude plugin answers with,
// runs modal patterns 0-2 and knows nothing about a boot screen. Claude's own
// ready marker is "❯" (its plugin advertises exactly that), so any Claude boot
// screen that numbers its options behind that glyph — the onboarding theme
// picker is the candidate — reproduces this incident on the plugin this repo
// requires for sessions to start at all. UNVERIFIED in the sense that matters:
// no Claude boot screen has been captured, which is precisely why no clause was
// written for one. BOS-894 deferred it deliberately (see the Open Questions in
// its plan); the gate below covers whatever the claude plugin answers, so the
// work is a capture plus a plugin-side clause and nothing here changes. It is
// written down at the binding site because every test being green is exactly
// what hides a gap of this shape.
//
// Cost, since an earlier reading of this path over-estimated it: the detector is
// probed at most once per readiness wait — on the capture that resolved a
// composer row, or on the last capture when the deadline expires — not once per
...
func (l *Lifecycle) injectTmuxChatInput(ctx context.Context, tmuxName string, input ChatInput, cmdResp *bossanovav1.BuildInteractiveCommandResponse, client agent.AgentRunnerClient, agentName string) error {
	...
	if input.Delivery == DeliverySubmit {
		...
		if err := l.tmux.SendPlanWithModal(ctx, tmuxName, prompt, marker, detector); err != nil {
			return fmt.Errorf("send plan to tmux session %q: %w", tmuxName, err)
		}
		return nil
	}
	if input.Command != "" {
		if err := l.tmux.PrefillLineWithModal(ctx, tmuxName, prompt, marker, detector); err != nil {
			return fmt.Errorf("prefill command to tmux session %q: %w", tmuxName, err)
		}
		return nil
	}
	if err := l.tmux.PrefillPlanWithModal(ctx, tmuxName, prompt, marker, detector); err != nil {
...
func bossSessionContext(f SessionFacts) string {
	...
	// No mcp__boss__* claim. bossd does not configure MCP servers for a chat —
	// each harness discovers them natively and the repo declares them — so it
	// cannot know whether this chat has the boss server, and a prompt that
	// asserted it would be advertising tools the agent may not have. BOSS_MCP_BIN
	// above is a different thing: it is an env export that still happens, and the
	// idVars list must keep mirroring managedSessionEnv exactly.
