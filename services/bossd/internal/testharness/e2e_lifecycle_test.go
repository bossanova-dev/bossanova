...
func TestE2E_CreateSession_LinearBranchDuplicateReturnsExistingSession(t *testing.T) {
	ctx := context.Background()
	h := testharness.New(t)
	defer h.Close()
	...
	branch := "dave/won-470-add-regression-test-for-ssemanager-removed-guard"
	req := &pb.CreateSessionRequest{
		RepoId:     repoID,
		Title:      "[WON-470] Add regression test for sseManager removed-guard",
		Plan:       "Linear issue",
		BaseBranch: "dev",
		BranchName: &branch,
	}
	...
	if active[0].BranchName != branch {
		t.Fatalf("active branch = %q, want %q", active[0].BranchName, branch)
	}
}
...
func TestE2E_CreateSession_LinearBranchDuplicateWhileStartingReturnsAlreadyExists(t *testing.T) {
	...
	h.Git.CreateFromExistingBranchFunc = func(ctx context.Context, opts gitpkg.CreateFromExistingBranchOpts) (*gitpkg.CreateResult, error) {
		...
		return &gitpkg.CreateResult{WorktreePath: t.TempDir(), BranchName: opts.BranchName}, nil
	}
	...
	go func() {
		...
		if err != nil {
			firstDone <- createResult{err: fmt.Errorf("create session: %w", err)}
			return
		}
		defer stream.Close() //nolint:errcheck // test cleanup
		var sess *pb.Session
		...
		if err := stream.Err(); err != nil {
			firstDone <- createResult{err: fmt.Errorf("create session stream: %w", err)}
			return
		}
		if sess == nil {
			firstDone <- createResult{err: fmt.Errorf("missing SessionCreated")}
			return
		}
		firstDone <- createResult{session: sess}
	}()
	<-started
